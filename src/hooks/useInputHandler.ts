...
import { routeInputWithContext, showPubkySelectionSheet, handleNoPubkysAvailable } from './inputHandlerUtils';
...
export const useInputHandler = (options: UseInputHandlerOptions = {}): UseInputHandlerReturn => {
	...
	const handleInput = useCallback(
		async (data: string, source: InputSource, overridePubky?: string): Promise<void> => {
			...
			try {
				...
				if (actionRequiresPubky(parsed.action) && !effectivePubky) {
					...
					const selectedPubky = await showPubkySelectionSheet(parsed, source, dispatch);
					if (selectedPubky) {
	...
	const showScanner = useCallback(
		async (scannerOptions?: ScannerOptions): Promise<void> => {
